...
Captures the scene at a specific time and returns it as a **base64-encoded PNG data-URL**. The output image is always `opts.width × opts.height` pixels, regardless of the current viewport zoom.
...
You can call `renderFrame` while the studio is paused without affecting playback state:

```ts
// Generate a thumbnail every 2 seconds
const durationMs = studio.maxDuration / 1000; // µs → ms
const thumbs: string[] = [];

for (let t = 0; t <= durationMs; t += 2000) {
  thumbs.push(await studio.renderFrame(t));
}
```
...
- [Working with Clips](../core/clips) - Load and configure media
- [Animations](./animations) - Add motion to your videos
- [Transitions](./transitions) - Create smooth transitions
- [Rendering and Export](../core/rendering) - Export video and extract frames
