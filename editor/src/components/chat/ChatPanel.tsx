...
import { Plan } from "../../types/director.types";
...
export const ChatPanel: React.FC<{ projectId: string; token: string }> = ({ projectId, token }) => {
  ...
  useEffect(() => {
    ...
    const unsubPlanCreated = newClient.on("plan.created", (plan: Plan) => {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: `I have a plan: ${plan.goal}. Do you approve?` },
      ]);
    });
    ...
    const unsubPlanComplete = newClient.on("plan.complete", () => {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: `Plan completed successfully!` },
      ]);
    });
  ...
  const handleRejectPlan = () => {
    if (pendingPlan && client) {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "Plan rejected. What would you like to do instead?" },
      ]);
    }
  };
    ...
    <div className="flex flex-col h-full bg-zinc-900 border-l border-zinc-800 w-80 text-sm">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((m, i) => (
          <div
            key={i}
            className={`p-2 rounded ${m.role === "user" ? "bg-blue-600 ml-8" : "bg-zinc-800 mr-8"}`}
          >
            {m.content}
          </div>
        ))}
        ...
        {pendingPlan && (
          <div className="p-3 border border-yellow-600 bg-yellow-900/20 rounded mr-8 space-y-2">
            ...
            <div className="flex space-x-2 pt-2">
              ...
              <Button size="sm" variant="ghost" onClick={handleRejectPlan}>
                Reject
              </Button>
            </div>
          </div>
